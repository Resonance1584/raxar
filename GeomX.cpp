...
Point3 Point3::operator+(Vector3 rhs){
    return Point3(this->getX() + rhs.getXDir(), this->getY() + rhs.getYDir(), this->getZ() + rhs.getZDir());
}

Vector3 Point3::operator-(Point3 rhs) {

	//So confused by this one
    //return Vector3(rhs.getX() - this->getX(), rhs.getY()  - this->getY(), rhs.getZ() - this->getZ());
	return Vector3(this->getX() - rhs.getX(),  this->getY() - rhs.getY(), this->getZ() - rhs.getZ());
}

bool Point3::operator==(Point3 rhs){
    if( fabs(this->x - rhs.x) < epsilon &&
...
double Vector3::getXDir(){
    return this->dirX;
}
double Vector3::getYDir(){
    return this->dirY;
}
double Vector3::getZDir(){
    return this->dirZ;
}
...
Vector3 Vector3::operator+(Vector3 rhs){
	return Vector3(this->dirX + rhs.getXDir(), this->dirY + rhs.getYDir(), this->dirZ + rhs.getZDir());
}
Vector3 Vector3::operator-(Vector3 rhs){
	return Vector3(this->dirX - rhs.getXDir(), this->dirY - rhs.getYDir(), this->dirZ - rhs.getZDir());
}
...
double Vector3::dot(Vector3 rhs){
	return this->dirX * rhs.getXDir() + this->dirY * rhs.getYDir() + this->dirZ * rhs.getZDir();
}
...
double Vector3::length(){
	return sqrt(this->dirX*this->dirX + this->dirY*this->dirY + this->dirZ*this->dirZ);
}
...
Ray3::Ray3(Point3 startP, Vector3 directionV){
    ...
    direction = Vector3(directionV.getXDir(), directionV.getYDir(), directionV.getZDir());
}
