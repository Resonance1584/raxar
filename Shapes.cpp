...
Vector3 Triangle::normal(Point3 p){
	return this->internalPlane.normal(p);
}
...
Vector3 Square::normal(Point3 p){
	return this->internalPlane.normal(p);
}
...
Point3 Square::getPoint(){
	return internalPlane.getPoint();
}
...
Cube::Cube(Point3 p, double size, Material mat){
	...
	Point3 frontLeft = Point3(p.getX() - width/2, p.getY() - width/2, p.getZ() + width/2);
	Point3 backRight = frontLeft + (right(d) + up(d) + far(d));

	//Front
	squares.push_back(new Square(frontLeft,frontLeft + right(d) + up(d),near(d),mat));
	//Left
	squares.push_back(new Square(frontLeft,frontLeft + far(d) + up(d), left(d),mat));
	//Back
	squares.push_back(new Square(backRight,backRight + down(d) + left(d),far(d),mat));
	//Right
	squares.push_back(new Square(backRight,backRight + near(d) + down(d),right(d),mat));
	//Top
	squares.push_back(new Square(backRight,backRight + near(d) + left(d),up(d),mat));
	//Bot
	squares.push_back(new Square(frontLeft,frontLeft + far(d) + right(d),down(d),mat));

	
	//Improves chances of getting both intersections in a row (by about a second, dont forget worst case still occurs)
	random_shuffle(squares.begin(),squares.end());

	lastHit = squares.front();
}

Vector3 Cube::normal(Point3 p){
	return lastHit->normal(p);
}
...
double Cube::intersect(Ray3 r){
	...
	for(vector<Square*>::iterator it = squares.begin(); it != squares.end(); it++){
		if (hits >= 2) break; //makes about 1 seconds difference
		Square* itShape = *it;
...
void Cube::removeBackFaces(Point3 eyePoint){
	...
	for(vector<Square*>::iterator it = squares.begin(); it != squares.end(); it++){
		...
		if(dot(itShape->normal(itShape->getPoint()),v) >= 0){ //Our check that it is visible
...
Vector3 Polyhedron::normal(Point3 p){
	return lastHit->normal(p);
}
...
double Polyhedron::intersect(Ray3 r){
	...
	for(vector<Triangle*>::iterator it = polys.begin(); it != polys.end(); it++){
		Triangle* tri = *it;
