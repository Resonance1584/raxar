...
//Material Colours
#define SHINY_RED Material(Colour(0.7,0.3,0.2),Colour(0.4,0.4,0.4),100,0.25,1.0,1.0)
#define SHINY_BLUE Material(Colour(0.2,0.3,0.7),Colour(0.8,0.8,0.8),200,0.5,1.0,1.0)
#define MATT_GREEN Material(Colour(0.1,0.7,0.1),Colour(0,0,0),0,0,1.0,1.0)
#define MATT_BLUE Material(Colour(0.1,0.1,0.7),Colour(0,0,0),0,0,1.0,1.0)
#define MATT_CYAN Material(Colour(0.1,0.7,0.7),Colour(0,0,0),0,0,1.0,1.0)
...
//Mirror material
#define MIRROR Material(Colour(0.5,0.5,0.5),Colour(0,0,0),0,0.9,1.0,1.0)
...
int main() {
	...
	//Spotlight
	lights.push_back(new SpotLight(LIGHT_INTENS, LIGHT_DIR, AMBIENT, Point3(2,4,2), 8));
	lights.push_back(new SpotLight(LIGHT_INTENS, unit(Vector3(0,1,1)), AMBIENT, Point3(-1,2,2), 18));
	//Scene definition
	list<Shape*> scene;
	...
	scene.push_back(new Triangle(Point3(1,0,0),Point3(2,0,0),Point3(1.5,1,0),MATT_GREEN));
	...
	//scene.push_back(new Triangle(Point3(-1.0,2.0,-1.5),Point3(0,0,-1.5),Point3(1.0,2.0,-2.5),MIRROR));


	vector<Triangle*> pyramid;
	pyramid.push_back(new Triangle(Point3(-1,0,0),Point3(0,0,1),Point3(0,1,0),SHINY_BLUE));
	pyramid.push_back(new Triangle(Point3(0,0,1),Point3(1,0,0),Point3(0,1,0),SHINY_BLUE));
	pyramid.push_back(new Triangle(Point3(1,0,0),Point3(0,0,-1),Point3(0,1,0),SHINY_BLUE));
	pyramid.push_back(new Triangle(Point3(0,0,-1),Point3(-1,0,0),Point3(0,1,0),SHINY_BLUE));
	...
	scene.push_back(new Square(Point3(-3,0,0),Point3(0,3,-3),Vector3(1,0,1),MIRROR));
	scene.push_back(new Square(Point3(0,3,-3),Point3(3,0,0),Vector3(-1,0,1),MIRROR));
	...
	for(float x = 0.0f; x < WIDTH; x++){
		...
		for(float fragmenty = y; fragmenty < y + 1.0f; fragmenty += 0.5f)
		{
			...
			do {
				...
				Shape* shape = NULL;
				for(list<Shape*>::iterator it = scene.begin(); it != scene.end(); it++){
					Shape* itShape = *it;
				...
				for(list<Lighting*>::iterator it = lights.begin(); it != lights.end(); it++){
					Lighting* light = *it;
					//Calculate shadowed
					bool isShadowed = false;

					//Generate a shadow ray
					Ray3 shadowRay = Ray3(hit+(unit(light->direction()) / 100), unit(light->direction()));
					...
					if(shape->isSphere() && shape->getMaterial().isTex()){
						...
						Vector3 vn = Vector3(0,1,0); //unit vector pointing up (vertical axis)
						Vector3 ve = Vector3(1,0,0); //unit vector pointing left (horizontal axis)
						Vector3 vp = unit(hit - shape->getPoint()); //unit vector from center to hit point (hit axis)
						...
						u = hit.getX();// / sqrt(pow(hit.getX(),2) + pow(hit.getY(),2) + pow(hit.getZ(),2));
						v = hit.getZ();// / sqrt(pow(hit.getX(),2) + pow(hit.getY(),2) + pow(hit.getZ(),2));
					}

					//Accumulate colour values
					colour = colour + (shape->getMaterial().lit_colour(hit, u, v, normal, light, -ray.directionV(), isShadowed) * coef);
				}
				...
				if(coef > 0.0){
					Vector3 reflection = ( normal * (ray.directionV().dot(normal)) * -2) + ray.directionV();
