...
class SpotLight: public Lighting{
	...
	double attn;
public:
	SpotLight(double intensity, Vector3 direction, double ambient, Point3 origin, double attenuation);
 	double intensity(Point3 p);
...
public:
	...
	Material(Colour diffuse, Colour specular, double shininess, double reflectiveness, double alpha, double refractiveIndex);
	//Constructs a Material with a texture file
	Material(STGA texture, Colour specular, double shininess, double reflectiveness, double transparency, double refractiveIndex);
	//Calculates the colour at a given point
	Colour lit_colour(Point3 pos, double u, double v, Vector3 normal, Lighting* light, Vector3 view, bool isShadowed);
	Colour diffuse();
	Colour specular();
	double shininess();
	double reflCoef();
