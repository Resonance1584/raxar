...
DirectionLight::DirectionLight(double intensity, Vector3 direction, double ambient){
	this->lightIntensity = intensity;
	this->lightDirection = direction;
	this->ambientIntensity = ambient;
}

double DirectionLight::intensity(Point3 p){
	return lightIntensity;
}

SpotLight::SpotLight(double intensity, Vector3 direction, double ambient, Point3 origin, double attenuation){
	this->lightIntensity = intensity;
	this->lightDirection = direction;
	this->ambientIntensity = ambient;
	this->origin = origin;
...
Material::Material(Colour diffuse, Colour specular, double shininess, double reflectiveness, double alpha, double refractiveIndex){
...
Material::Material(STGA texture, Colour specular, double shininess, double reflectiveness, double alpha, double refractiveIndex){
...
Colour Material::lit_colour(Point3 pos, double u, double v, Vector3 normal, Lighting* light, Vector3 view, bool isShadowed){
	...
	//Kd * Is diffuse
	i += (diffuseColour * light->intensity(pos)) * max(0.0, (light->direction().dot(normal)));
	...
	i += (specularColour * light->intensity(pos)) * pow(max(0.0, h.dot(normal)), shininessAmount);
...
double Material::shininess(){
	return shininessAmount;
}

double Material::reflCoef(){
	return reflectCoef;
}
...
Colour Material::posColour(double u, double v, STGA texture){
	...
	//Get the index in 1d array for our position, multiply by 3 to account for each channel
